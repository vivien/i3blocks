...
For the lazy, you can start from 
link:https://github.com/vivien/i3blocks-contrib[our collection of scripts], if you did not installed {progname} via your package manager:
...
If the value of the block's interval is _persist_, then the data is written of the command standard input, one line per click.
...
There are plenty in the link:https://github.com/vivien/i3blocks-contrib[contrib repository].
...
Yes, any variable defined in the block is exported in the environment of its command.
The `foo=bar` property can be accessed with `$foo` from a shell script, `ENV["foo"]` from Ruby, and so on.

However note that the definitions in the configuration file are just raw data, {progname} does not do string interpolation of any sort.
This means that `bar=$baz` defines a _bar_ variable equal to literally `$baz` (dollar sign + "baz").
...
This is pretty straightforward actually. Just make sure not to override the _full_text_, for example:
